...
class Settings:
    ...
    def __init__(self, json_file: str = DEFAULT_SETTING_FILE) -> None:
        ...
        self.upstream_proxy: str = self._get_value("upstream_proxy",
                                                   "")  # mitm upstream proxy server e.g. http://ip:port
        ...
        self.language: str = self._get_value("language", list(LAN_OPTIONS.keys())[-1],
                                             self.valid_language)  # language code
        ...
        self.reverse_allocation_probability_threshold: float = self._get_value(
            "reverse_allocation_probability_threshold", 0.5, lambda x: 0 <= x <= 1)
        self.reverse_allocation_probability_degree: float = self._get_value("reverse_allocation_probability_degree",
                                                                            0.1, lambda x: 0 <= x <= 1)
        self.completely_random_probability: float = self._get_value("completely_random_probability", 0.05,
                                                                    lambda x: 0 <= x <= 1)
        self.probability_truncation_threshold: float = self._get_value("probability_truncation_threshold", 0.025,
                                                                       lambda x: 0 <= x <= 1)
        ...
        self.auto_retry_interval: float = self._get_value("auto_retry_interval", 1.5,
                                                          lambda x: 0.5 < x < 30.0)  # not shown
