...
class GameState:
            ...
            if 'data' in liqi_data:
                if 'operation' in liqi_data['data']:
                    ...
                    if liqi_data['data']['operation']['seat'] != self.seat:
                        LOGGER.warning("operation seat %s != self.seat %s", liqi_data['data']['operation']['seat'],
                                       self.seat)
    ...
    def _react_all(self, data=None) -> dict | None:
        ...
        """
        if data:
            if 'operation' not in data or 'operationList' not in data['operation'] or len(
                    data['operation']['operationList']) == 0:
                return None
        try:
