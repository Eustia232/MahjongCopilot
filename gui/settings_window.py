...
class SettingsWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        select_menu = ttk.Combobox(main_frame, textvariable=self.model_type_var, values=MODEL_TYPE_STRINGS,
                                   state="readonly", width=std_wid)
        select_menu.grid(row=cur_row, column=1, **args_entry)
        ...
        select_menu = ttk.Combobox(main_frame, textvariable=self.model_file_var, values=model_files, state="readonly",
                                   width=std_wid * 3)
        ...
        select_menu2 = ttk.Combobox(main_frame, textvariable=self.model_file_3p_var, values=model_files,
                                    state="readonly", width=std_wid * 3)
        ...
        sel_model = ttk.Combobox(main_frame, textvariable=self.mjapi_model_select_var, values=options, state="readonly",
                                 width=std_wid)
        ...
        idle_move_entry = ttk.Checkbutton(main_frame, variable=self.auto_idle_move_var,
                                          text=self.st.lan().AUTO_IDLE_MOVE, width=std_wid)
        idle_move_entry.grid(row=cur_row, column=2, **args_entry)
        # drag dahai
        self.auto_drag_dahai_var = tk.BooleanVar(value=self.st.auto_dahai_drag)
        _entry = ttk.Checkbutton(main_frame, variable=self.auto_drag_dahai_var, text=self.st.lan().DRAG_DAHAI,
                                 width=std_wid)
        ...
        options = [i/10.0 for i in range(10,-1,-1)]
        ...
        _label.grid(row=cur_row, column=0, **args_label)
        self.reverse_allocation_probability_threshold = tk.DoubleVar(
            value=self.st.reverse_allocation_probability_threshold)
        options = [i/10.0 for i in range(10,-1,-1)]
        random_choice_entry = ttk.Combobox(
            main_frame, textvariable=self.reverse_allocation_probability_threshold, values=options, state="readonly",
            width=std_wid)
        random_choice_entry.grid(row=cur_row, column=1, **args_entry)
        ...
        options = [0, 0.05, 0.1, 0.15, 0.2,0.25,0.3]
        random_choice_entry = ttk.Combobox(
            main_frame, textvariable=self.reverse_allocation_probability_degree, values=options, state="readonly",
            width=std_wid)
        ...
        options = [i * 0.01 for i in range(0, 10)]
        random_choice_entry = ttk.Combobox(
            main_frame, textvariable=self.completely_random_probability, values=options, state="readonly",
            width=std_wid)
        random_choice_entry.grid(row=cur_row, column=1, **args_entry)
        ...
        random_choice_entry = ttk.Combobox(
            main_frame, textvariable=self.probability_truncation_threshold, values=options, state="readonly",
            width=std_wid)
        random_choice_entry.grid(row=cur_row, column=3, **args_entry)
    ...
    def _on_save(self):
        ...
        self.st.reverse_allocation_probability_threshold = self.reverse_allocation_probability_threshold.get()
        self.st.reverse_allocation_probability_degree = self.reverse_allocation_probability_degree.get()
        self.st.completely_random_probability = self.completely_random_probability.get()
        self.st.probability_truncation_threshold = self.probability_truncation_threshold.get()
        ...
        if self.mitm_proxinject_updated:
            messagebox.showinfo(self.st.lan().SETTINGS, self.st.lan().SETTINGS_TIPS, parent=self, icon='info',
                                type='ok')
        self.destroy()
